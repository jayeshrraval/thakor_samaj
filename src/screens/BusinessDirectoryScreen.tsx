...
export default function BusinessDirectoryScreen() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24 font-gujarati">
      ...
      {/* Header */}
      <div className="bg-[#800000] pt-6 pb-4 px-6 rounded-b-[30px] shadow-lg sticky top-0 z-10">
        ...
        <div className="flex bg-black/10 p-1 rounded-xl mt-4 backdrop-blur-md">
          <button onClick={() => { resetForm(); }} className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'list' ? 'bg-white text-[#800000] shadow-md' : 'text-white/80'}`}>વ્યાપાર ડાયરેક્ટરી</button>
          <button onClick={() => setActiveTab('add')} className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'add' ? 'bg-white text-[#800000] shadow-md' : 'text-white/80'}`}>{editingId ? "✏️ સુધારો (Edit)" : "+ રજીસ્ટ્રેશન"}</button>
        </div>
      </div>
        ...
        {activeTab === 'list' && (
          <>
            <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-hide mb-2">
              <button onClick={() => setSelectedCategory('All')} className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${selectedCategory === 'All' ? 'bg-[#800000] text-white' : 'bg-white text-gray-600 border border-gray-200'}`}>બધા (All)</button>
              {businessTypes.map(type => (
                <button key={type} onClick={() => setSelectedCategory(type)} className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${selectedCategory === type ? 'bg-[#800000] text-white' : 'bg-white text-gray-600 border border-gray-200'}`}>{type.split(' ')[0]}</button>
              ))}
            </div>

            <div className="space-y-4">
              {loading ? ( <div className="flex justify-center py-10"><Loader2 className="animate-spin text-[#800000]" size={32} /></div> ) : filteredList.length === 0 ? (
                  ...
                  <div key={item.id} onClick={() => navigate(`/business-details/${item.id}`)} className="relative cursor-pointer bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden transform transition-all active:scale-[0.99]">
                    ...
                    {currentUserId === item.user_id && (
                        <div className="absolute top-2 right-2 flex gap-1 z-20">
                            <button onClick={(e) => handleEdit(e, item)} className="p-2 bg-blue-50 text-red-900 rounded-full shadow-sm hover:bg-blue-100"><Edit2 size={14} /></button>
                        </div>
                    )}

                    <div className="bg-gray-50 p-4 border-b border-gray-100 flex justify-between items-start">
                      <div>
                        <div className="text-[10px] font-bold text-[#800000] bg-[#800000]/10 px-2 py-0.5 rounded-md w-fit mb-1">{item.business_type}</div>
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex gap-2 mt-2">
                        <a href={`tel:${item.mobile}`} onClick={callNow} className="flex-1 bg-[#800000] text-white py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 shadow-sm active:bg-[#157a8c]"><Phone size={16} /> Call</a>
          ...
          <form onSubmit={handleRegister} className="bg-white p-6 rounded-3xl shadow-lg border border-gray-100 space-y-4">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input required placeholder="ઉદા. જય ગણેશ ટ્રેડર્સ" className="w-full bg-gray-50 p-3.5 rounded-2xl outline-none border border-gray-200 focus:border-[#800000]"
            ...
            <button disabled={loading} type="submit" className="w-full bg-[#800000] text-white py-4 rounded-2xl font-bold text-lg shadow-lg shadow-cyan-200 mt-6 active:scale-95 transition-transform flex items-center justify-center gap-2">
